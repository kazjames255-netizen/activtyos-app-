...
export function ParentAccidentsApp() {
  ...
  const refresh = useCallback(() => {
    ...
    apiGet<Rec[]>("/api/incidents").then((r) => { setRecords([...r].sort((a, b) => `${b.date ?? ""}${b.time ?? ""}`.localeCompare(`${a.date ?? ""}${a.time ?? ""}`))); setError(null); }).catch((e) => setError(e instanceof Error ? e.message : "care.accLoadFailed")); // i18n key, translated at render (keeps this callback free of `t`)
  }, []);
  ...
  const TAB = tab === "incident"
    ? { key: "incident" as const, c: "#6d28d9", bg: "#f3e8ff", icon: "🧩", label: t("care.accTabIncidents"), empty: t("care.accEmptyIncidents") }
    : { key: "accident" as const, c: "#c02636", bg: "#fdebec", icon: "⛑️", label: t("care.accTabAccidents"), empty: t("care.accEmptyAccidents") };

  async function acknowledge(id: string) {
    setAcking(id); setError(null);
    try { await apiPost(`/api/incidents/${encodeURIComponent(id)}/acknowledge`, {}); refresh(); }
    catch (e) { setError(e instanceof Error ? e.message : t("care.accSaveFailed")); }
    ...
    <div className="-m-5 min-h-[calc(100vh-3.5rem)] bg-[var(--bg)] p-5 text-[var(--ink)]" style={LIGHT_PALETTE}>
      <div className="relative mb-3.5 overflow-hidden rounded-2xl p-5 text-white shadow-[0_10px_30px_-12px_rgba(29,58,143,.55)]" style={{ background: "linear-gradient(120deg,#1d3a8f 0%,#3f78d8 100%)" }}>
        <div className="flex items-center gap-2 text-[22px] font-extrabold" style={{ fontFamily: "var(--ff-display)" }}>
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/20 text-[17px]">⛑️</span>{t("care.accTitle")}
        </div>
        <p className="mt-1.5 max-w-[560px] text-[12.5px] leading-[1.5] text-white/85">{t("care.accLede")}</p>
        <div className="mt-4 flex flex-wrap gap-1.5">
          {([["accident", "⛑️", t("care.accTabAccidents"), nAcc], ["incident", "🧩", t("care.accTabIncidents"), nInc]] as [("accident" | "incident"), string, string, number][]).map(([k, ic, lbl, n]) => (
      ...
      {records && outstanding.length > 0 && (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border-2 border-[#f0b100] bg-[#fff8e6] px-4 py-3">
          <span className="text-[13px] font-bold text-[#8a5a00]">{outstanding.length === 1 ? t("care.accBannerOne") : t("care.accBannerMany", { n: outstanding.length })}</span>
          <button type="button" onClick={() => { setOpenId(outstanding[0].id); document.getElementById(`rec-${outstanding[0].id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }); }} className="rounded-full bg-[#1d3a8f] px-4 py-1.5 text-[12.5px] font-extrabold text-white">{t("care.accReviewBtn")}</button>
        </div>
      )}
      ...
      {records && (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3.5 py-2.5">
          <span className="text-[12.5px] text-[var(--ink-2)]">{muted ? t("care.accAlertsOff") : t("care.accAlertsOn")}</span>
          <button type="button" onClick={toggleMute} className="rounded-full border px-3.5 py-1.5 text-[12px] font-bold transition-colors" style={muted ? { borderColor: "#1d3a8f", color: "#1d3a8f" } : { borderColor: "var(--line)", color: "var(--ink-2)" }}>{muted ? t("care.accAlertsTurnOn") : t("care.accAlertsTurnOff")}</button>
        </div>
      )}

      {error && <div className="mb-3 rounded-lg border border-[var(--red-line,#f6c9cc)] bg-[var(--red-soft,#fdebec)] px-3 py-2 text-[12.5px] text-[var(--red,#e21d27)]">{error.startsWith("care.") ? t(error) : error}</div>}

      {!records ? <div className="py-10 text-center text-[12.5px] text-[var(--ink-3)]">{t("care.loading")}</div>
              ...
              <Card key={r.id} id={`rec-${r.id}`} className="overflow-hidden p-0">
                ...
                <div className="p-4">
                  <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                        ...
                        {isBehaviour(r) ? <span className="rounded-full bg-[#eef4fd] px-2 py-0.5 text-[11px] font-bold text-[#1d3a8f]">🧩 {t("care.accBehaviour")}{r.incidentType ? ` · ${r.incidentType}` : ""}</span> : r.injury && <span className="text-[13px] text-[var(--ink-2)]">{r.injury}</span>}
                      </div>
                      {r.description && <p className="mt-1 max-w-[640px] text-[13px] leading-snug text-[var(--ink-2)]">{r.description}</p>}
                    </div>
                    <div className="flex flex-col items-start gap-1.5 sm:items-end">
                      <Badge tone={{ bg: sev.bg, fg: sev.fg }}>{t(sev.labelKey)}</Badge>
                      <span className="text-[11.5px] font-bold text-[var(--ink-2)]">{fmtDate(r.date, r.time, loc)}</span>
                      {(r.parentNotified || r.parentNotifiedAt) && <Badge tone={{ bg: "#eaf0fc", fg: "#1d3a8f" }}>{t("care.accNotified")}{r.parentNotifiedAt ? ` · ${stamp(r.parentNotifiedAt, loc)}` : ""}</Badge>}
                      {r.updatedAt && <Badge tone={{ bg: "#fdf3d8", fg: "#9a5a00" }}>{t("care.accUpdated", { when: stamp(r.updatedAt, loc) })}</Badge>}
                      {r.acknowledgedAt && <Badge tone={{ bg: "#e7f6ee", fg: "#0f7a43" }}>{t("care.accAcked", { when: stamp(r.acknowledgedAt, loc) })}</Badge>}
                    </div>
                  </div>
                  <div className="mt-3 border-t border-[var(--line)] pt-2.5">
                    <button type="button" onClick={() => setOpenId(openId === r.id ? null : r.id)} className="text-[12px] font-bold text-[#1d3a8f] hover:underline">{openId === r.id ? t("care.accHideDetails") : t("care.accShowDetails")}</button>
                  </div>
                  {openId === r.id && (
                  ...
                  <div className="mt-3 rounded-xl border border-[#cfe0f7] bg-[#f4f8ff] p-3">
                    <div className="text-[12px] font-extrabold text-[#1d3a8f]">{t("care.accMessageTitle")}</div>
                    <p className="mt-0.5 text-[11px] text-[var(--ink-3)]">{t("care.accMessageHint")}</p>
                  ...
                  {r.acknowledgedAt ? (
                    <div className="mt-3 flex items-center gap-2 rounded-xl border border-[#cfe9d8] bg-[#eef8f1] px-3.5 py-2.5 text-[12.5px] font-semibold text-[#0f7a43]">
                      {t("care.accAckedOn", { when: stamp(r.acknowledgedAt, loc) })}
                    </div>
                  ) : (
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#cfe0f7] bg-[#f4f8ff] px-3.5 py-2.5">
                      <span className="text-[12.5px] font-semibold text-[var(--ink-2)]">{t("care.accAckPrompt")}</span>
                      <button type="button" disabled={acking === r.id} onClick={() => acknowledge(r.id)}
                        ...
                        {acking === r.id ? t("care.accSaving") : t("care.accAckBtn")}
                      </button>
                    </div>
                  )}
                </div>
