...
const ADDABLE: { icon: string; label: string; hint: string; make: () => Block | Block[] }[] = [
  ...
  { icon: "⏰", label: "Countdown timer", hint: "Counts down to your date", make: () => { const d = new Date(Date.now() + 14 * 86400000); const p = (n: number) => String(n).padStart(2, "0"); return B("countdown", { heading: "Early-bird ends in", label: "Book before it's gone", date: `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`, time: "18:00" }); } },
...
export function CampaignDesigner({ initial, company, socials, onCancel, onSave, saveLabel }: { initial?: CampaignDesign | null; company?: Partial<Company>; socials?: Social[]; onCancel: () => void; onSave: (d: CampaignDesign) => void; saveLabel?: string }) {
