  ...
  switch (b.t) {
    ...
    case "stats": {
      ...
      if (sv === "cards") {
        ...
        return row(`<table role="presentation" width="100%" style="border-collapse:collapse;width:100%"><tr>${inner}</tr></table>`, "padding:18px 16px");
      }
      if (sv === "band") {
        ...
        return row(`<table role="presentation" width="100%" style="border-collapse:collapse;width:100%;background:${t.a};border-radius:16px"><tr>${inner}</tr></table>`, "padding:16px 20px");
      }
      if (sv === "circles") {
        ...
        return row(`<table role="presentation" width="100%" style="border-collapse:collapse;width:100%"><tr>${inner}</tr></table>`, "padding:20px 16px");
      }
...
export function CampaignDesigner({ initial, company, socials, onCancel, onSave }: { initial?: CampaignDesign | null; company?: Partial<Company>; socials?: Social[]; onCancel: () => void; onSave: (d: CampaignDesign) => void }) {
  ...
  const blockEditor = (b: Block) => {
    ...
    switch (b.t) {
      ...
      case "countdown": return <div className="space-y-1.5">{hd("Heading", b.heading)}<div className="grid grid-cols-2 gap-2"><div><div className={lbl}>Date it counts down to</div><input type="date" value={(b.date || "").split("T")[0]} onChange={(e) => patch(k, { date: e.target.value })} className={inputCls} /></div><div><div className={lbl}>Start time</div><input type="time" step={60} value={b.time ?? ((b.date || "").split("T")[1] || "").slice(0, 5)} onChange={(e) => patch(k, { time: e.target.value })} className={inputCls} /></div></div><div className="text-[10px] text-[var(--ink-3)]">Counts down to the day &amp; start time above — e.g. when your session begins. Shows days · hrs · mins · secs, frozen at the moment the email is sent (email can&apos;t tick live).</div>{hd("Button label (optional)", b.label, "label")}{hd("Button link", b.url, "url")}</div>;
