...
export const TOUR_STEPS: Record<string, LiveTourSteps> = {
  ...
  // Schedule: drive the REAL rota end-to-end — add a shift, auto-assign, note &
  // save, auto-schedule the week, request availability, fill it in for someone,
  // then publish. Cursor-only, no spotlight boxes.
  schedule: {
    title: "Schedule & rota",
    noSpotlight: true,
    introLine: "This is your rota. Let me build a shift, staff the week, sort everyone's availability and send it out — start to finish.",
    doneLine: "Shift added, week auto-staffed, availability chased and filled in, and the rota published to your team. That's the whole loop.",
    steps: [
      { find: "Total wages", line: "Up top: the week's wage bill — at the hourly rate and including on-cost." },
      { find: "First Aider", line: "The rota's grouped by listing, with a row per role. Blue means assigned, red still needs someone." },
      { find: "First Aider", clickIn: [["＋", "First Aider"]], line: "Let's add a shift — I'll tap the plus on a First Aider day, and the editor opens." },
      { find: "Assign staff", click: true, line: "Every shift needs someone on it. I'll open Assign staff." },
      { find: "Auto-fill available staff", pick: ["Auto-fill available staff"], line: "Auto-fill instantly picks whoever's free and not already booked that day — no hunting.", advance: "Done" },
      { find: "Add shift note", click: true, fill: [["Shift note", "Bring the first-aid kit"]], line: "Add a note the staff member will see on their shift, then save it.", advance: "Save" },
      { find: "Auto-schedule", click: true, pick: ["Fill open shifts"], line: "Don't fill them one by one — Auto-schedule staffs every open shift for the week from your confirmed, available team in one go." },
      { find: "Request staff to confirm availability", click: true, pick: ["This week"], line: "Not sure who's free? Ask them. This sends everyone who hasn't submitted a request to fill in their availability — for this week or all weeks." },
      { find: "Jordan Lee", click: true, pick: ["Weekdays 9"], line: "You can also fill it in for someone — open their card and set the days they can work. I'll mark Jordan as weekdays.", advance: "Done" },
      { find: "Publish to staff", click: true, line: "When the week's ready, Publish sends every assigned person their shifts — by email or push, whichever you set — and locks them in." },
      { find: "Settings", click: true, line: "And the Settings tab tunes the lot: publish alerts, shift reminders, the check-in grace and availability auto-chasing." },
    ],
  },
  dash: {
    title: "Dashboard",
