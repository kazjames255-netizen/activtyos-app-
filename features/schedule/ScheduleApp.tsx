...
export function ScheduleApp() {
  ...
  const overdueLabel = (s: Shift) => { const m = overdueMin(s); if (m < 60) return t("schedule.minLate", { min: m }); const h = Math.floor(m / 60); return t("schedule.hmLate", { h, m: m % 60 }); };
  ...
  const remind = (id: string) => { persist({ ...store, staff: store.staff.map((s) => (s.id === id ? { ...s, reminders: (s.reminders ?? 0) + 1 } : s)) }); flash(t("schedule.reminderSent")); };
  // Per-shift check-in nudge (separate from the availability reminder count).
  const pokeCheckin = (shiftId: string) => { persist({ ...store, shifts: store.shifts.map((s) => (s.id === shiftId ? { ...s, checkinPokes: (s.checkinPokes ?? 0) + 1 } : s)) }); flash(t("schedule.checkinReminderSent")); };
  // Ask ONE staff member to complete their availability (from their editor).
  const requestOne = (id: string, scope: "this" | "all") => { persist({ ...store, staff: store.staff.map((s) => (s.id === id ? { ...s, requested: true, requestedScope: scope, requestedAt: Date.now(), reminders: (s.reminders ?? 0) + 1 } : s)) }); flash(t("schedule.availRequestedScope", { scope: scope === "this" ? label : t("schedule.allWeeksLower") })); };
  ...
  const requestAvail = (scope: "this" | "all") => {
    ...
    if (!targets.length) { flash(t("schedule.everyoneConfirmed")); return; }
    const at = Date.now();
    ...
    flash(t("schedule.availRequestedFrom", { count: targets.length, scope: scope === "this" ? label : t("schedule.allWeeksThisSeason") }));
  };
  ...
  function copyToAllDays() {
    ...
    setDraft(null); setActionsOpen(false); flash(t("schedule.copiedToDays", { count: targets.length }));
  }
  function duplicateShift() {
    if (!draft) return;
    const others = store.shifts.filter((s) => !draft.groupIds.includes(s.id));
    persist({ ...store, shifts: [...others, ...currentRows(draft), ...freshRows(draft, draft.date, "d")] });
    setDraft(null); setActionsOpen(false); flash(t("schedule.shiftDuplicated"));
  }
  ...
  function autoFill() {
    ...
    flash(filled ? t("schedule.autoFilledShifts", { count: filled }) : t("schedule.noConfirmedFree"));
  }
  function clearPeriod() { setAutoMenu(false); const n = periodShifts.length; persist({ ...store, shifts: store.shifts.filter((s) => !inPeriod(s)) }); flash(t("schedule.clearedShifts", { count: n })); }
  function copyForward() { setCopyMenu(false); const len = dates.length; const copies = periodShifts.map((s, i) => ({ ...s, id: `sh${Date.now()}${i}`, date: addDays(s.date, len), in: undefined, out: undefined, locked: false })); persist({ ...store, shifts: [...store.shifts, ...copies] }); flash(t("schedule.copiedForward", { count: copies.length, period: span === "day" ? t("schedule.periodDay") : span === "month" ? t("schedule.periodMonth") : span === "week" ? t("schedule.periodWeek") : span === "2w" ? t("schedule.period2w") : t("schedule.period4w") })); }
  function saveTemplate() {
    ...
    setTplSaveOpen(false); setTplName(""); flash(t("schedule.savedTemplate", { name, count: items.length }));
  }
  function applyTemplate(tpl: Template) {
    ...
    persist({ ...store, shifts: [...store.shifts, ...created] }); setTplListOpen(false); flash(t("schedule.appliedTemplate", { name: tpl.name, count: created.length }));
  }
  function deleteTemplate(id: string) { persistTpl(templates.filter((t) => t.id !== id)); }
  function publish() { const ids = new Set(periodShifts.filter((s) => s.staffId).map((s) => s.id)); persist({ ...store, shifts: store.shifts.map((s) => (ids.has(s.id) ? { ...s, locked: true } : s)) }); const how = notifyOnPublish === "off" ? t("schedule.howNoNotif") : notifyOnPublish === "email" ? t("schedule.howEmail") : notifyOnPublish === "push" ? t("schedule.howPush") : t("schedule.howEmailPush"); flash(t("schedule.publishedToStaff", { count: assignedStaff.size, how })); }
  ...
  return (
    <div className="-m-3 min-h-[calc(100vh-3.5rem)] p-3 sm:-m-5 sm:p-5" style={LIGHT_PALETTE}>
      <PageHero title={t("schedule.staffScheduleTitle")} icon="🗓" lede={t("schedule.staffScheduleLede")} />
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-6">
          ...
          <div className="flex items-stretch gap-3">
            <div className="rounded-xl bg-white/10 px-4 py-2.5 ring-1 ring-white/15 backdrop-blur">
              <div className="text-[9.5px] font-extrabold uppercase tracking-[0.08em] text-white/60">{t("schedule.atHourlyRate")}</div>
            ...
            <div className="rounded-xl bg-white px-4 py-2.5 shadow-lg ring-1 ring-white/40">
              <div className="text-[9.5px] font-extrabold uppercase tracking-[0.08em] text-[#5566a0]">{t("schedule.inclOnCost", { pct: onCost })}</div>
      ...
      <div className="mb-3 inline-flex rounded-xl bg-[var(--panel)] p-1">
        {([["rota", t("schedule.tabRota")], ["attendance", t("schedule.tabAttendance")], ["availability", t("schedule.tabAvailability")], ["settings", t("schedule.tabSettings")]] as const).map(([v, lbl]) => (
      ...
      {schedView === "attendance" ? <StaffAttendanceBoard /> : schedView === "availability" ? <AvailabilityRequestsPanel /> : schedView === "settings" ? <SchedulingSettingsForm /> : (
      <>
      ...
      <div className="flex flex-col gap-3 lg:flex-row">
          ...
          <Card className="p-2.5">
            ...
            <div className="mb-2 flex items-center gap-1">
              ...
              <Select value={staffSort} onChange={(e) => setStaffSort(e.target.value as typeof staffSort)} className="ml-auto w-full max-w-[150px] text-[11px]">
                ...
                <option value="costHigh">{t("schedule.sortCostHigh")}</option>
                <option value="costLow">{t("schedule.sortCostLow")}</option>
              </Select>
            </div>
            {canManage && <div className="mb-2.5">
              <div className="mb-1 text-[9px] font-extrabold uppercase tracking-[0.05em] text-[var(--ink-3)]">{t("schedule.step1Confirm")}</div>
              <button type="button" onClick={() => setReqOpen((v) => !v)} className="w-full rounded-lg bg-[#c0392b] px-2 py-2.5 text-[10.5px] font-extrabold uppercase leading-tight tracking-wide text-white shadow-sm hover:brightness-105">{t("schedule.requestStaffConfirm")}{notSubmitted ? ` · ${notSubmitted}` : ""}</button>
              {reqOpen && <div className="mt-1.5">
                <div className="mb-1 text-[9px] font-extrabold uppercase tracking-[0.05em] text-[var(--ink-3)]">{t("schedule.sendRequestFor")}</div>
                <div className="grid grid-cols-2 gap-1.5">
                  <button type="button" onClick={() => requestAvail("this")} className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-2 py-1.5 text-[11px] font-extrabold text-[var(--ink-2)] transition-colors hover:border-[#1d3a8f] hover:bg-[#eef4fd] hover:text-[#1d3a8f]">{t("schedule.thisWeek")}</button>
                  <button type="button" onClick={() => requestAvail("all")} className="rounded-lg border border-[var(--line)] bg-[var(--surface)] px-2 py-1.5 text-[11px] font-extrabold text-[var(--ink-2)] transition-colors hover:border-[#1d3a8f] hover:bg-[#eef4fd] hover:text-[#1d3a8f]">{t("schedule.allWeeks")}</button>
                </div>
              </div>}
              {notSubmitted === 0 && store.staff.length > 0 && <div className="mt-1.5 text-center text-[10px] font-bold text-[#0f7a43]">✓ {t("schedule.allNConfirmed", { n: store.staff.length })}</div>}
            </div>}
            <div className="flex flex-col divide-y divide-[var(--line-2,#eef2f8)]">
              {shownStaff.map((st) => { const hrs = staffHours(st.id); const pay = hrs * st.rate; const cost = pay * (1 + onCost / 100); return (
                <div key={st.id}
                  ...
                  role="button"
                  title={t("schedule.clickToEditAvail")}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 text-[11px] font-bold text-[var(--ink)]">{money(pay)} <span className="font-normal text-[var(--ink-3)]">· {money(cost)} {t("schedule.onCostSuffix")}</span></div>
                  </div>
                </div>
              ); })}
            </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mb-2 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-xl bg-white px-3.5 py-2 text-[11.5px] font-bold text-[var(--ink-2)] shadow-[0_1px_3px_rgba(16,24,64,0.08)] ring-1 ring-black/[0.04]">
            <span className="text-[10px] font-extrabold uppercase tracking-wide text-[var(--ink-3)]">{t("schedule.key")}</span>
                  ...
                  {gridListings.map((l) => {
                    ...
                    <div key={l.venueId + "|" + l.title}>
                      ...
                      {canManage && <div className="border-b border-[var(--line-2,#eef2f8)] px-3 py-2.5">
                        <button type="button" onClick={() => setRoleMenu(l.title)} className="text-[13px] font-extrabold text-[#1d3a8f] hover:underline">＋ {t("schedule.addNewRole")}</button>
                      </div>}
                    </div>
                  ); })}
                  </>
        ...
        <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-black/45 p-4 pt-[8vh]" onClick={() => setShowAlerts(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            {alerts.length === 0 ? <p className="mt-3 rounded-lg bg-[#e2f4ea] px-3 py-2.5 text-[12.5px] font-bold text-[#0f7a43]">✓ {t("schedule.noOneOverdue")}</p> : (() => {
              ...
              return (<>
              <input value={alertQ} onChange={(e) => setAlertQ(e.target.value)} placeholder={t("schedule.searchStaffName")} className="mt-3 w-full rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-[12.5px] text-[var(--ink)] outline-none focus:border-[var(--brand)]" />
              ...
              {shown.length === 0 && <p className="py-4 text-center text-[12px] text-[var(--ink-3)]">{t("schedule.noOverdueMatch", { q: alertQ })}</p>}
              </div>
              </>);
            })()}
          </div>
        ...
        <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-black/45 p-4 pt-[10vh]" onClick={() => setTplSaveOpen(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setTplSaveOpen(false)} className="rounded-full border border-[var(--line)] bg-white px-4 py-1.5 text-[12.5px] font-bold text-[var(--ink-2)]">{t("schedule.cancel")}</button>
              <button type="button" disabled={!tplName.trim() || periodShifts.length === 0} onClick={saveTemplate} className="rounded-full bg-[#1d3a8f] px-4 py-1.5 text-[12.5px] font-extrabold text-white disabled:opacity-40">{t("schedule.saveTemplateBtn")}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-black/45 p-4 pt-[8vh]" onClick={() => setTplListOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            {templates.length === 0 ? (
              ...
              <div className="mt-3 flex flex-col gap-1.5">{templates.map((tpl) => (
                <div key={tpl.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--panel)] px-3 py-2.5 text-[12.5px]">
                  <div className="min-w-0 flex-1"><div className="truncate font-bold text-[var(--ink)]">{tpl.name}</div><div className="text-[11px] text-[var(--ink-3)]">{tpl.items.length} {tpl.items.length === 1 ? t("schedule.shift") : t("schedule.shifts")}</div></div>
                  <button type="button" onClick={() => applyTemplate(tpl)} className="rounded-full bg-[#1d3a8f] px-3 py-1 text-[11.5px] font-bold text-white hover:bg-[#16306e]">{t("schedule.apply")}</button>
                  <button type="button" onClick={() => deleteTemplate(tpl.id)} title={t("schedule.deleteTemplate")} className="rounded-full border border-[var(--line)] bg-white px-2.5 py-1 text-[11.5px] font-bold text-[#c0392b] hover:bg-[#fdebec]">{t("schedule.delete")}</button>
                </div>
              ))}</div>
            )}
          </div>
            ...
            <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="px-5 py-4">
                ...
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  <button type="button" onClick={() => preset(["mon", "tue", "wed", "thu", "fri"])} className="rounded-full border border-[var(--line)] bg-white px-3 py-1.5 text-[12px] font-bold text-[var(--ink-2)] hover:bg-[var(--panel)]">{t("schedule.weekdays95")}</button>
                  <button type="button" onClick={() => preset(WDAYS.map(([k]) => k))} className="rounded-full border border-[var(--line)] bg-white px-3 py-1.5 text-[12px] font-bold text-[var(--ink-2)] hover:bg-[var(--panel)]">{t("schedule.all7Days")}</button>
                  <button type="button" onClick={() => commit({})} className="rounded-full border border-[var(--line)] bg-white px-3 py-1.5 text-[12px] font-bold text-[var(--ink-2)] hover:bg-[var(--panel)]">{t("schedule.clearAll")}</button>
                </div>
                {/* per-day */}
                ...
                <div className="mt-4 flex justify-end"><Button variant="primary" onClick={() => setAvailEdit(null)}>{t("schedule.done")}</Button></div>
              </div>
            </div>
          </div>
        );
      ...
      {seasonMenu && (
        <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-black/45 p-4 pt-[9vh]" onClick={() => setSeasonMenu(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end"><button type="button" onClick={() => setSeasonMenu(false)} className="rounded-full bg-[#1d3a8f] px-6 py-2 text-[13.5px] font-extrabold text-white hover:bg-[#16306e]">{t("schedule.done")}</button></div>
          </div>
        </div>
      )}
