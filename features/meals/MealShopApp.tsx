...
export function MealShopApp({ canManage, onBoard }: { canManage: boolean; onBoard: () => void }) {
  ...
  const [f, setF] = useState({ name: "", price: "", description: "", allergens: [] as string[] });
  const set = (patch: Partial<typeof f>) => setF((p) => ({ ...p, ...patch }));
  const toggleAllergen = (a: string) => setF((p) => ({ ...p, allergens: p.allergens.includes(a) ? p.allergens.filter((x) => x !== a) : [...p.allergens, a] }));
  ...
  async function addOption() {
    ...
    try { await apiPost("/api/meal-options", { name: f.name, price, description: f.description || undefined, allergens: f.allergens }); setF({ name: "", price: "", description: "", allergens: [] }); setOpen(false); refresh(); }
      ...
      {open && (
        <Card className="mb-3 p-3.5">
          ...
          <div className="mt-2.5">
            <FieldLabel>Contains (allergens)</FieldLabel>
            <div className="flex flex-wrap gap-1">
              {UK_ALLERGENS.map((a) => (
                <button key={a} type="button" onClick={() => toggleAllergen(a)}
                  className="rounded-full border px-2 py-[2px] text-[11px] font-bold capitalize transition-colors"
                  style={f.allergens.includes(a) ? { borderColor: "transparent", background: "var(--red,#e21d27)", color: "#fff" } : { borderColor: "var(--line)", color: "var(--ink-3)" }}>
                  {a}
                </button>
              ))}
            </div>
        ...
        <div className="mb-5 flex flex-col gap-1.5">
          {options.map((o) => (
            <Card key={o.id} className="flex flex-wrap items-center gap-2 p-2.5">
              ...
              {(o.allergens?.length ?? 0) > 0 && <span className="text-[10.5px] font-bold capitalize text-[var(--red,#e21d27)]">contains {o.allergens!.join(", ")}</span>}
