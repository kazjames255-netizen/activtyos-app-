...
// Gmail-style bright backgrounds for the Messages surface. Each theme sets the
// page wash (shows around the white conversation cards) and a matching hero
// band; the cards themselves stay opaque white so text always reads. "Classic"
// is the original blue/pink wash. Choice is per-browser (localStorage).
interface MsgTheme { id: string; name: string; swatch: string; page: string; hero: string }
const MESSAGE_THEMES: MsgTheme[] = [
  {
...
const THEME_STORAGE_KEY = "aos.msgTheme";
...
export function MessagesApp({ mode }: { mode: "operator" | "parent" }) {
  ...
  // Bright background theme for the Messages surface (Gmail-style). Starts on
  // "classic" for a stable first render, then hydrates the saved choice from
  ...
  useEffect(() => {
    try { const s = localStorage.getItem(THEME_STORAGE_KEY); if (s && MESSAGE_THEMES.some((t) => t.id === s)) setThemeId(s); } catch { /* private mode */ }
  }, []);
  ...
  return (
    ...
    <div
      ...
      style={{
        ...
        // cleanly on top; the wash shows around and beneath them.
                  ...
                  <div className="absolute right-0 z-50 mt-2 w-[280px] rounded-2xl border border-[var(--line)] bg-white p-3 text-left shadow-[0_18px_40px_-12px_rgba(20,20,60,.4)]">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {MESSAGE_THEMES.map((t) => {
                        ...
                        return (
                          <button key={t.id} type="button" onClick={() => pickTheme(t.id)} className="group flex flex-col gap-1 rounded-xl p-1 text-left transition-colors hover:bg-[var(--panel)]">
                            <span className="relative block h-11 w-full overflow-hidden rounded-lg ring-1 ring-black/5" style={{ background: t.swatch, boxShadow: on ? "0 0 0 2px var(--brand-2)" : undefined }}>
                              {on && <span className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-white text-[10px] font-extrabold text-[var(--brand-2)] shadow">✓</span>}
                            </span>
