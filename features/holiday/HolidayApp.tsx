      ...
      {tab === "allowances" && (
        <Card className="mt-4 p-4">
          ...
          <div className="overflow-x-auto rounded-xl border border-[var(--line)]">
            ...
            <tbody>{profiles.map((p) => { const s = summaryOf(p.id); const dpw = p.daysPerWeek ?? policy.daysPerWeek; const custom = p.allowanceDays != null || policy.allowanceBasis === "custom"; const pct = s.total > 0 ? Math.round(((s.takenAnnual + s.bookedAnnual) / s.total) * 100) : 0; return (
              <tr key={p.id} className="border-t border-[var(--line-2,#eef2f8)]">
                ...
                <td className="px-3 py-2.5 text-center">{custom ? <span className="rounded-full bg-[#eef4fd] px-2 py-0.5 text-[10px] font-bold text-[#1d3a8f]">Contractual</span> : <span className="rounded-full bg-[#eef7ee] px-2 py-0.5 text-[10px] font-bold text-[#0f7a43]">Statutory</span>}</td>
                ...
                <td className="px-3 py-2.5 text-right"><div className="font-extrabold tabular-nums text-[#0f7a43]">{s.remaining}</div><div className="mt-0.5 h-1 w-14 overflow-hidden rounded-full bg-[#e6ebf3]"><div className="h-full rounded-full bg-[#1d3a8f]" style={{ width: `${Math.min(100, pct)}%` }} /></div></td>
