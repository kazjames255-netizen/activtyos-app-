...
function StatusPill({ h, now, kind }: { h: HW; now: number; kind: boolean }) {
  ...
  if (s === "submitted") return <Pill tone="brand" icon={<Ico name="hourglass" size={12} />}>{kind ? "Handed in" : "Awaiting marking"}</Pill>;
  const ds = dueState(h.dueAt, s, now, kind);
...
export function StudentHomework({ qs, childId, students, topics, onError }: PanelProps) {
  ...
  const load = useCallback((force = false) => {
    getShared<HW[]>(path, force).then((r) => { if (!mounted.current) return; setFailed(false); setList(Array.isArray(r) ? r : []); })
      .catch((e) => { if (mounted.current) { setFailed(true); onError(errMsg(e, "Couldn't load your homework")); } });
  }, [path, onError]);
  ...
  if (list === null && failed) return <RetryFace id="hub-homework" what="your homework" kid={kid} onRetry={retry} />;
  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)] gap-4" id="hub-homework">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {[
          { label: "To do", n: groups.todo.length, sub: kid ? (overdue ? "waiting for you" : "all on track") : overdue ? `${overdue} overdue` : "all on track", tone: overdue && !kid ? "var(--red)" : "var(--brand-2)" },
          { label: kid ? "Handed in" : "Awaiting marking", n: groups.waiting.length, sub: "handed in", tone: "var(--brand)" },
    ...
    <div className="@container grid gap-4" id="hub-homework-detail" data-status={sub.status}>
      ...
      <section className="relative overflow-hidden rounded-3xl p-5 text-white shadow-[var(--shadow)] sm:p-6" style={HERO_BG}>
        <div className="flex flex-wrap items-center gap-2 text-[11px] font-bold">
          {marked ? <span className="rounded-full bg-[var(--green)] px-2.5 py-[3px] uppercase tracking-wide">Marked</span> : sub.status === "submitted" ? <span className="rounded-full bg-white/20 px-2.5 py-[3px] uppercase tracking-wide">{kid ? "Handed in" : "Awaiting marking"}</span> : <span className={`rounded-full px-2.5 py-[3px] uppercase tracking-wide ${ds.overdue && !kid ? "bg-[var(--red)]" : "bg-white/20"}`}>{ds.label}</span>}
          {sub.late && <span className={`rounded-full px-2.5 py-[3px] uppercase tracking-wide ${kid ? "bg-white/20" : "bg-[var(--red)]"}`}>{kid ? "Handed in" : "Handed in late"}</span>}
        </div>
        ...
        {marked && sub.mark && (
          <div className="mt-4 rounded-2xl border border-white/20 bg-white/12 p-4 backdrop-blur-sm" data-testid="hub-mark">
            <div className="flex flex-wrap items-end gap-x-4 gap-y-1">
              ...
              {!kid && <div className="pb-1 text-[15px] font-bold text-white/90">{pctOf(sub.mark)}%</div>}
            </div>
        ...
        <section className="content-start rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)]" aria-label="Your hand-in">
          ...
          {marked || (!editing && sub.status !== "assigned") ? (
            ...
            <div className="grid gap-3">
              ...
              {ds.overdue && sub.status === "assigned" && !kid && <p className="text-[11.5px] font-semibold text-[var(--red)]">This is past its due date — it&rsquo;ll be marked as late.</p>}
            </div>
          )}
        </section>
      </div>
