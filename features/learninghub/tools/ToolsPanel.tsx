...
// The Tools tab: every learning tool, by subject and key stage. Working tools open; the rest are shown greyed ("In build" / "Coming soon")
// rather than hidden, and a click on one is counted (anonymously) so the tools people ask for get built first.
...
const BADGE = { live: null, building: "In build", soon: "Coming soon" } as const;
...
function Card({ t, onOpen }: { t: ToolMeta; onOpen: (t: ToolMeta) => void }) {
  const idle = t.status !== "live";
  return (
    <li>
      <button type="button" onClick={() => onOpen(t)} aria-disabled={idle || undefined} data-status={t.status} data-testid={`tool-${t.id}`}
        className={`flex h-full w-full flex-col gap-2 rounded-2xl border bg-[var(--surface)] p-3.5 text-left shadow-[var(--shadow-sm)] transition-shadow hover:shadow-md motion-reduce:transition-none ${FOCUS} ${idle ? "border-dashed border-[var(--line)] opacity-60" : "border-[var(--line)]"}`}>
        <span className="flex items-start gap-2">
          <span className="min-w-0 flex-1 text-[14.5px] font-extrabold leading-snug text-[var(--ink)]">{t.title}</span>
          {BADGE[t.status] && <span className="flex-none rounded-full border border-dashed border-[var(--ink-3)] px-2 py-px text-[10.5px] font-extrabold uppercase tracking-wide text-[var(--ink-2)]">{BADGE[t.status]}</span>}
        </span>
...
export function Panel({ qs, childQs }: PanelProps) {
  ...
  const counts = useMemo(() => Object.fromEntries(SUBJECT_ORDER.map((s) => [s, REGISTRY.filter((t) => t.subject === s).length])) as Record<ToolSubject, number>, []);
  const shown = useMemo(() => {
    const w = text.trim().toLowerCase();
    return REGISTRY.filter((t) => (subject === "all" || t.subject === subject) && (!ks || t.keyStages.includes(ks)) && (!readyOnly || t.status === "live") && (!w || `${t.title} ${t.tags.join(" ")}`.toLowerCase().includes(w)))
      .sort((a, b) => Number(b.status === "live") - Number(a.status === "live") || a.title.localeCompare(b.title));
  }, [subject, ks, text, readyOnly]);
  const live = REGISTRY.filter((t) => t.status === "live").length;

  const onOpen = (t: ToolMeta) => {
    if (t.status === "live") { logToolEvent(q0, t.id, "open"); setNote(null); setOpen(t); return; }
    logToolEvent(q0, t.id, t.status === "building" ? "buildingClick" : "comingSoonClick");
    setNote(`“${t.title}” isn’t ready yet — ${t.status === "building" ? "it’s being built" : "it’s planned for later"}. We’ve noted you’d like it.`);
  };
  ...
  return (
    <div id="hub-tools">
      <div className="mb-4 flex flex-wrap items-center gap-2.5">
        ...
        <span className="text-[12.5px] font-semibold text-[var(--ink-2)]" aria-live="polite">{shown.length} {shown.length === 1 ? "tool" : "tools"} · {live} ready to use</span>
        <label className="ml-auto inline-flex min-h-[44px] cursor-pointer items-center gap-2 text-[13px] font-bold text-[var(--ink)]"><input type="checkbox" checked={readyOnly} onChange={(e) => setReadyOnly(e.target.checked)} className="h-4 w-4 accent-[var(--brand)]" />Ready to use only</label>
      </div>
      ...
      {note && <p role="status" className="mb-3 rounded-xl bg-[var(--panel)] px-3.5 py-2.5 text-[13px] font-bold text-[var(--ink)]">{note}</p>}
