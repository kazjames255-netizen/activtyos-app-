...
export function ParentMomentsApp() {
  ...
  return (
    <div className="text-[var(--ink)]">
      <h2 className="mb-1 text-[22px] font-extrabold" style={{ fontFamily: "var(--ff-display)" }}>{t("feed.momentsTitle")}</h2>
      <p className="mb-3 text-[12.5px] text-[var(--ink-3)]">{t("feed.momentsLede")}</p>

      <div className="mb-3 flex flex-wrap gap-1.5">
        {([["all", t("feed.folderAll", { n: all.length })], ["child", t("feed.folderChild", { n: counts.child })], ["work", t("feed.folderWork", { n: counts.work })]] as const).map(([k, l]) => (
      ...
      {error !== null && <div className="mb-3 text-[12.5px] text-[var(--red,#e21d27)]">{error || t("feed.loadFailed")}</div>}
      {!moments ? <div className="py-10 text-center text-[12.5px] text-[var(--ink-3)]">{t("feed.loading")}</div>
        : shown.length === 0 ? <Card className="p-6 text-center text-[13px] text-[var(--ink-3)]">{all.length === 0 ? t("feed.momentsEmpty") : t("feed.folderEmpty")}</Card>
        : (
                  ...
                  {(m.comments?.length ?? 0) > 0 && (
                    <div className="mt-2 border-t border-[var(--line)] pt-2">
                      {(m.comments ?? []).map((c, idx) => (
                        <div key={idx} className="mb-1 flex items-start gap-1.5 text-[11.5px] leading-[1.5]"><span className="flex-none font-bold" style={{ color: c.role === "parent" ? BLUE : "var(--ink)" }}>{c.role === "parent" ? t("feed.you") : c.byName}:</span><span className="flex-1 text-[var(--ink-2)]">{c.text}</span></div>
                      ))}
                    </div>
                  )}
