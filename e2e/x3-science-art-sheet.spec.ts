...
// Slide-picture review sheet (X3: Science extension pictures; copy of f1-art-sheet.spec.ts with its own samples/output): signs in as the Oak STAGING tutor (scratch/oak-staging.json), opens sampled lessons in the Lessons tab,
// plays the Preview and, for every sampled slide, asserts the PICTURE POLICY as rendered and screenshots the slide into scratch/x3-art-shots/.
...
// Samples: scratch/x3-art-samples.json (made from the factory check output by /tmp scripts; see docs/hub-review/F1-images.md).
// Run ONLY via scripts/e2e-locked.sh e2e/x3-science-art-sheet.spec.ts   (X3_LIMIT=n limits the number of lessons; X3_THEME=light|dark; X3_WIDTH=390 for a phone)
...
const samples: Sample[] = (() => { try { return JSON.parse(fs.readFileSync(path.join(ROOT, "scratch/x3-art-samples.json"), "utf8")); } catch { return []; } })().slice(0, Number(process.env.X3_LIMIT ?? 999));
const OUT = path.join(ROOT, `scratch/x3-art-shots${process.env.X3_THEME === "light" ? "-light" : ""}${process.env.X3_WIDTH ? `-w${process.env.X3_WIDTH}` : ""}`);
...
test.skip(!staging || !samples.length, "needs scratch/oak-staging.json and scratch/x3-art-samples.json");

test("sampled slides render exactly their verified pictures (or no art panel)", async ({ browser }) => {
  test.setTimeout(90 * 60_000);
  fs.mkdirSync(OUT, { recursive: true });
  const ctx = await browser.newContext({ viewport: { width: Number(process.env.X3_WIDTH ?? 1100), height: 900 }, colorScheme: process.env.X3_THEME === "light" ? "light" : "dark" });
