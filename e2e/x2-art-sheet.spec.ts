...
// Samples: scratch/x2-art-samples.json (made from the factory check output by /tmp scripts; see docs/hub-review/F1-images.md).
// Run ONLY via scripts/e2e-locked.sh e2e/x2-art-sheet.spec.ts   (X2_LIMIT=n limits the number of lessons; X2_THEME=light|dark; X2_WIDTH=390 for a phone)
...
const samples: Sample[] = (() => { try { return JSON.parse(fs.readFileSync(path.join(ROOT, "scratch/x2-art-samples.json"), "utf8")); } catch { return []; } })().slice(0, Number(process.env.X2_LIMIT ?? 999));
const OUT = path.join(ROOT, `scratch/x2-art-shots${process.env.X2_THEME === "light" ? "-light" : ""}${process.env.X2_WIDTH ? `-w${process.env.X2_WIDTH}` : ""}`);
...
test.skip(!staging || !samples.length, "needs scratch/oak-staging.json and scratch/x2-art-samples.json");

test("sampled slides render exactly their verified pictures (or no art panel)", async ({ browser }) => {
  test.setTimeout(90 * 60_000);
  fs.mkdirSync(OUT, { recursive: true });
  const ctx = await browser.newContext({ viewport: { width: Number(process.env.X2_WIDTH ?? 1100), height: 900 }, colorScheme: process.env.X2_THEME === "light" ? "light" : "dark" });
