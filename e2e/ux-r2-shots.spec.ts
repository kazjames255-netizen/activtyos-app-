...
for (const vp of ONLY) {
  test.describe(`${vp}`, () => {
    ...
    test(`in-person capture grid (${vp})`, async ({ browser }) => {
      ...
      await expect(app.getByTestId("ip-chosen")).toContainText(L.title, { timeout: 40_000 });
      await shot(page, `ip-${vp}-setup`); await tapAudit(page, `ip-${vp}-setup`);
      await app.getByRole("button", { name: avaName, exact: true }).click();
      await app.getByRole("button", { name: benName, exact: true }).click();
