...
// Samples: scratch/f1-art-samples.json (made from the factory check output by /tmp scripts; see docs/hub-review/F1-images.md).
// Run ONLY via scripts/e2e-locked.sh e2e/f1-art-sheet.spec.ts   (F1_LIMIT=n limits the number of lessons; F1_THEME=light|dark; F1_WIDTH=390 for a phone)
...
const samples: Sample[] = (() => { try { return JSON.parse(fs.readFileSync(path.join(ROOT, "scratch/f1-art-samples.json"), "utf8")); } catch { return []; } })().slice(0, Number(process.env.F1_LIMIT ?? 999));
const OUT = path.join(ROOT, `scratch/f1-art-shots${process.env.F1_THEME === "light" ? "-light" : ""}${process.env.F1_WIDTH ? `-w${process.env.F1_WIDTH}` : ""}`);
...
test.skip(!staging || !samples.length, "needs scratch/oak-staging.json and scratch/f1-art-samples.json");

test("sampled slides render exactly their verified pictures (or no art panel)", async ({ browser }) => {
  test.setTimeout(90 * 60_000);
  fs.mkdirSync(OUT, { recursive: true });
  const ctx = await browser.newContext({ viewport: { width: Number(process.env.F1_WIDTH ?? 1100), height: 900 }, colorScheme: process.env.F1_THEME === "light" ? "light" : "dark" });
