...
// plays the Preview and, for every sampled slide, asserts the PICTURE POLICY as rendered and screenshots the slide into scratch/x5-lang-shots/.
...
// Samples: scratch/x5-languages-samples.json (made from the factory check output by /tmp scripts; see docs/hub-review/F1-images.md).
// Run ONLY via scripts/e2e-locked.sh e2e/f1-art-sheet.spec.ts   (X5_LIMIT=n limits the number of lessons; X5_THEME=light|dark; X5_WIDTH=390 for a phone)
...
const samples: Sample[] = (() => { try { return JSON.parse(fs.readFileSync(path.join(ROOT, "scratch/x5-languages-samples.json"), "utf8")); } catch { return []; } })().slice(0, Number(process.env.X5_LIMIT ?? 999));
const OUT = path.join(ROOT, `scratch/x5-lang-shots${process.env.X5_THEME === "light" ? "-light" : ""}${process.env.X5_WIDTH ? `-w${process.env.X5_WIDTH}` : ""}`);
...
test.skip(!staging || !samples.length, "needs scratch/oak-staging.json and scratch/x5-languages-samples.json");

test("sampled slides render exactly their verified pictures (or no art panel)", async ({ browser }) => {
  test.setTimeout(90 * 60_000);
  fs.mkdirSync(OUT, { recursive: true });
  const ctx = await browser.newContext({ viewport: { width: Number(process.env.X5_WIDTH ?? 1100), height: 900 }, colorScheme: process.env.X5_THEME === "light" ? "light" : "dark" });
