...
//   node scripts/leads/find_contacts.mjs [--limit N] [--only id1,id2] [--apply]      (from server/; state in scripts/leads/out/batch1_contacts.out.jsonl)
...
const LIMIT = args.limit ? +args.limit : Infinity; const ONLY = args.only ? new Set(String(args.only).split(",")) : null; const OUT = path.resolve("scripts/leads/out/batch1_contacts.out.jsonl"); const CONC = 16, TIMEOUT = 12000, MAXBYTES = 800_000;
