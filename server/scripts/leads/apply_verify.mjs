...
for (const r of rows) { const ref = db.collection("leads").doc(r.id); const stamp = new Date().toISOString(); let upd=null;
  if (r.kind==="candidate") {
    if (r.verdict==="confirm" || r.verdict==="confirm-weak") { upd = { website: r.final || r.url, websiteFoundBy: `verified 13 Sept 2026: name + ${r.locHits?.[0]||"location"} + children's wording on the site`, websiteCandidate: D(), websiteCandidateWhy: D(), websiteRejected: D(), websiteRejectedWhy: D() }; bump("candidate→confirmed"); }
  ...
  if (accepted && r.booking) { const have = cur.get(r.id)?.bookingSystem; const b = r.booking;
    if (b.system && /Google Forms|Typeform|Jotform/.test(b.system) && !have) { upd.bookingChecked = true; upd.bookingNote = `registers families through a ${b.system.replace(" pay","")} form (${b.page || ""}) — no booking system`; bump("booking: sign-up form only (greenfield)"); }
    else if (b.system && !have) { upd.bookingSystem = b.system; upd.bookingUrl = b.url && /^https?:/.test(b.url) ? b.url : (b.page || r.final || r.url); upd.bookingFrom = `site crawl ${stamp.slice(0,10)} (${b.pagesRead} page${b.pagesRead === 1 ? "" : "s"} read${b.evidence ? `, ${b.evidence}` : ""})`; upd.bookingChecked = true; bump("booking platform found: "+b.system); }
    else if (!b.system && !have) { if (b.pagesRead >= 2 || (b.subpages === 0 && (r.textLen ?? 0) > 1500)) { upd.bookingChecked = true; upd.bookingCheckedPages = b.pagesRead; bump(b.pagesRead >= 2 ? "booking: none seen on "+b.pagesRead+" pages (enquiry only)" : "booking: homepage links to no booking/shop/class pages at all (enquiry only)"); } else { upd.bookingChecked = D(); bump("booking: only the homepage could be read — left as not checked"); } } }
