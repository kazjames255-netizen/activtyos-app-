...
for (const r of rows) { const ref = db.collection("leads").doc(r.id); const stamp = new Date().toISOString(); let upd=null;
  ...
  if (accepted && r.booking) { const have = cur.get(r.id)?.bookingSystem; const b = r.booking;
    ...
    else if (!b.system && !have) { if (b.pagesRead >= 2) { upd.bookingChecked = true; upd.bookingCheckedPages = b.pagesRead; bump("booking: none seen on "+b.pagesRead+" pages (enquiry only)"); } else { upd.bookingChecked = D(); bump("booking: only the homepage could be read — left as not checked"); } } }
