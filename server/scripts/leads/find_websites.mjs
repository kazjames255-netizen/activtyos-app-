...
function pick(lead, results) { const nt = tokens(lead.name); const nsq = squash(lead.name); if (!nt.length) return null;
  for (const r of results.slice(0, 8)) { let host; try { host = new URL(r.url).hostname.replace(/^www\./,""); } catch { continue; } if (SKIP.test(host) || SKIP.test(r.url)) continue;
    ...
    const tt = tokens(r.title); const titleHits = nt.filter(t => tt.includes(t)).length; const titleHit = nt.length ? titleHits / nt.length >= 0.6 : false;
...
async function one(d) { const l = d.data(); const q = `"${l.name}" ${town(l)}`.trim(); const r = await search(q); if (r.status === 429 || r.status === 403) { blocked++; console.log("blocked", r.status, "— backing off 90s"); await sleep(90000); return; }
  const p = r.status === 200 ? pick(l, r.results) : null; const row = { id: d.id, source: l.source, q, status: r.status, n: r.results.length, url: p?.url || null, why: p?.why || null, top: r.results.slice(0,3).map(x=>x.url.slice(0,80)) }; if (p) found++; out.write(JSON.stringify(row)+"\n"); if (++i % 100 === 0) console.log(i, "searched,", found, "candidates,", blocked, "blocked"); }
