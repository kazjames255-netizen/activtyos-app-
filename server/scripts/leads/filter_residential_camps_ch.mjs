// Research-only pass: residential/overnight kids' holiday camp operators (PGL/Camp Beaumont-style) from
// the Companies House free monthly bulk snapshot. These typically self-classify under the HOLIDAY
// ACCOMMODATION SIC codes (55201/55209), not any education/leisure code — genuinely unscanned by every
// prior pass. Same review-only pattern: writes a REVIEW file, does NOT write to Firestore.
//   node scripts/leads/filter_residential_camps_ch.mjs
...
// 55201 "Holiday centres and villages"; 55209 "Other holiday and other collective accommodation".
const TARGET_SIC = new Set(["55201", "55209"]);

// These two SIC codes are dominated by ordinary caravan parks, B&Bs, hostels and self-catering lets —
// require an explicit camp/kids-activity signal, not just "holiday accommodation".
const CAMP_WORDS = /\b(holiday camp|activity camp|adventure camp|residential camp|summer camp|kids? camp|children'?s? camp|scout camp|outdoor education( centre| center)?|outward bound|activity holidays?|adventure holidays?)\b/i;
const KIDS_WORDS = /\b(kids?|junior|juniors|youth|children'?s?|teen(s|agers?)?|school)\b/i;
function nameMatches(name) { return CAMP_WORDS.test(name) || (/\bcamp\b/i.test(name) && KIDS_WORDS.test(name)); }

const NOT_FIT_WORDS = /\b(caravan park|holiday park|static caravans?|lodges?|glamping|campsite pitches?|touring park|self[- ]?catering|b\s?&\s?b|bed and breakfast|guest ?house|hostel(?!.*(youth|kids?))|holdings?|investments?|propert(y|ies)|management services|letting agents?)\b/i;
function classifyFit(name) {
  const n = name.toLowerCase();
  if (NOT_FIT_WORDS.test(n)) return "likely_not_fit";
  if (!CAMP_WORDS.test(n)) return "uncertain"; // matched only via the looser "camp" + kids-word fallback
  return "likely_fit";
}
...
for (const f of ["sports_leads_review.json", "sports_leads_review_broadened.json", "sports_leads_all_high.json", "party_entertainment_leads_review.json", "daycare_leads_review.json", "performing_arts_leads_review.json", "farms_attractions_leads_review.json"]) {
...
function streamRows() {
  return new Promise((resolve, reject) => {
    ...
    parser.on("end", () => { console.log("rows scanned", total, "| SIC 55201/55209 Active matches", sicMatch, "| excluded (prior pass)", excludedPrior); resolve(high); });
...
console.log("residential-camp keyword matches (high confidence)", high.length);
...
const review = {
  ...
  source: `Companies House bulk snapshot ${ZIP} (SIC 55201/55209, residential-camp keywords, excludes prior CH passes)`,
  sicCodesUsed: [...TARGET_SIC],
  note: "These SIC codes are dominated by ordinary caravan parks, B&Bs and self-catering accommodation with " +
    "zero kids-activity angle, so an explicit camp/kids-activity phrase was required. Anything matched only " +
    "via the looser 'camp' + kids-word fallback (not an exact residential/activity-camp phrase) is flagged " +
    "uncertain, not likely_fit. Classification is name-only (no web lookups). NOT imported — review only.",
...
fs.writeFileSync(`${OUTDIR}/residential_camps_leads_review.json`, JSON.stringify(review, null, 2));
console.log("wrote", `${OUTDIR}/residential_camps_leads_review.json`);
