...
/** A reply to a lead-facing email (the pricing-page website-design
 * acknowledgement, HQ's question answer, a "you're booked in" demo
 * confirmation) carries a `lead-<id>@…` Reply-To — see
 * lib/emails.ts's leadReplySender. Checked BEFORE tenant resolution: these
 * addresses are generated by us and never collide with a tenant's own
 * slug/notify address, so there's nothing to disambiguate. */
async function resolveLeadReply(input: InboundInput): Promise<string | null> {
  // The local part is always-lowercase hex (leadReplySender/
  // ensureLeadReplyToken in lib/emails.ts) specifically so lowercasing
  // somewhere in transit — routine for many mail clients/providers — can
  // never break the match. Matched against a stored field, not the doc id
  // (Firestore ids are mixed-case and would reintroduce that exact bug).
  const addr = (input.to ?? "").toLowerCase();
  const m = addr.match(/^lead-([a-f0-9]+)@/);
  if (!m) return null;
  const hit = await db.collection("leads").where("replyToken", "==", m[1]).limit(1).get();
  return hit.empty ? null : hit.docs[0].id;
}

/** Files an inbound reply onto the lead as an activity (visible in the Sales
...
emailsInbound.post("/", async (req, res) => {
  ...
  const leadId = await resolveLeadReply(parsed.data);
  if (leadId) { await storeLeadReply(leadId, parsed.data); res.status(201).json({ ok: true, leadId }); return; }
...
emailsResendInbound.post("/", raw({ type: "application/json", limit: "5mb" }), async (req, res) => {
  ...
  const leadId = await resolveLeadReply(input);
  if (leadId) {
    await storeLeadReply(leadId, input);
    console.log(`[inbound] ${sender.email ?? sender.name} → ${deliveredTo} filed as a reply on lead ${leadId}`);
