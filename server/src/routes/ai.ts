...
const HOWTO: Record<"operator" | "staff" | "parent" | "platform" | "headoffice", string> = {
  ...
  operator: [
    ...
    "Add a discount code: Marketing → Discount codes → create a code (percentage or fixed, all listings or a specific one, optional per-family limit or reserved group).",
  ...
  staff: [
    ...
    "Time off & your shifts: Time off to request leave; My shifts & clock for your rota.",
  ...
  parent: [
    ...
    "Update your child's details: Child & details → edit allergies, medical, dietary, SEND/EHCP plan, consents and collection password.",
...
ai.post("/chat", async (req, res) => {
  ...
  const system = [
    ...
    "Money is in GBP — format amounts like £42.50. Be concise and concrete: lead with the answer, then only the supporting details that matter. FORMAT: plain text, short paragraphs, and simple '- ' bullet lists only. NEVER use markdown tables or pipes (|) — the app cannot render them, they show as raw text. Bold with **…** and links are fine.",
