...
/** Strict: the key must equal the recomputed value rounded to `sf` significant figures (so answers are quoted to the sf the prompt asks for). */
export function sfx(t: CTopic, key: string, calc: number, sf: number, need: string[] = []) {
  checks++;
  const q = find(t, key);
  const got = keyNum(q);
  const want = Number(calc.toPrecision(sf));
  if (Math.abs(got - want) > Math.abs(want) * 1e-9 + 1e-300) problems.push(`${key}: key ${got} but recomputed ${calc} → ${want} (${sf} s.f.)`);
  for (const n of need) if (!(q.prompt + " " + (q.options ?? []).join(" ")).includes(n)) problems.push(`${key}: prompt should contain "${n}"`);
  if (q.kind === "number" && (q.tolerance ?? -1) < 0) problems.push(`${key}: number question needs a tolerance`);
}
/** The right option text must equal the text the checker derived. */
export function opt(t: CTopic, key: string, want: string) {
  checks++;
  const got = String(find(t, key).answer);
  if (got !== want) problems.push(`${key}: right option "${got}" but derived "${want}"`);
}
/** Key = the option chosen by evaluating a condition (e.g. which value is larger). */
export const pick = (cond: boolean, a: string, b: string) => (cond ? a : b);
