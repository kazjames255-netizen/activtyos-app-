// Small builders shared by the German pack files (leading underscore = skipped by the validator).
// They assemble CQuestion objects: the right option goes in a rotating position, and a `short` question's
// accepted answers get ae/oe/ue/ss and plain-letter variants (na) when the task is not about umlauts or ß.
...
const plain = (s: string) => s.replace(/[äÄ]/g, (c) => (c === "ä" ? "a" : "A")).replace(/[öÖ]/g, (c) => (c === "ö" ? "o" : "O")).replace(/[üÜ]/g, (c) => (c === "ü" ? "u" : "U")).replace(/ß/g, "ss");
  ...
  return {
    ...
    short(prompt: string, answer: string, explanation: string, difficulty: D, o: { acc?: string[]; na?: boolean; diag?: boolean } = {}): CQuestion {
      ...
      for (const a of [answer, ...(o.acc ?? [])]) {
        set.add(a);
        if (o.na) { set.add(ascii(a)); set.add(plain(a)); }
      }
      set.delete(answer);
